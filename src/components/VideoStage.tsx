...
const getWordEmoji = (wordStr: string): string | null => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 bg-[#121419] border border-white/10 rounded-xl shadow-xl">
      ...
      <div className="mt-3 flex items-center gap-2.5 w-full bg-[#0B0C0F] px-3 py-2 rounded-xl border border-white/10 shadow-lg">
        <button
          onClick={onTogglePlay}
          className="flex h-8 w-8 items-center justify-center rounded-lg bg-brownie-500 text-black hover:scale-105 transition-transform font-bold shadow shrink-0"
        >
        ...
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-md bg-white/5 hover:bg-white/10 text-white/80 transition-colors shrink-0"
        ...
        <div className="flex-1 min-w-0">
          <input
            type="range"
            min="0"
            max={videoRef.current?.duration || (words.length > 0 ? words[words.length - 1].end + 1 : 10)}
            step="0.05"
            value={currentTime}
        ...
        <span className="font-mono text-xs font-semibold text-white/80 min-w-[75px] text-right tabular-nums shrink-0">
